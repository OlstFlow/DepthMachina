...
_DA3_COLLECTION_NAME = f"{_ADDON_PREFIX}_DA3"
_DA3_ENGINE_TAG = "da3"
...
def _find_group_root(*, collection: bpy.types.Collection, image_path_abs: str) -> bpy.types.Object | None:
    for obj in collection.objects:
        if obj.type != "EMPTY":
            continue
        try:
            if str(obj.get("dm_engine", "")) != _DA3_ENGINE_TAG:
                continue
...
def _ensure_group_root(*, collection: bpy.types.Collection, image_path_abs: str, base_name: str) -> tuple[bpy.types.Object, bool]:
    ...
    try:
        root["dm_engine"] = _DA3_ENGINE_TAG
...
def _ensure_group_camera(*, collection: bpy.types.Collection, image_path_abs: str, name: str) -> tuple[bpy.types.Object, bool]:
    ...
    if cam_obj is None:
        ...
        created = True
        try:
            cam_obj["dm_engine"] = _DA3_ENGINE_TAG
...
def _create_camera_from_payload(
    *,
    payload: dict,
    name: str,
    ...
    try:
        fov_x_deg = float(payload.get("fov_x_deg", 60.0) or 60.0)
    except Exception:
        fov_x_deg = 60.0
    try:
        fov_y_deg = float(payload.get("fov_y_deg", 45.0) or 45.0)
    except Exception:
        fov_y_deg = 45.0
    ...
    if image_w and image_h and image_h > image_w:
        ...
        cam_data.lens = cam_data.sensor_height / (2.0 * math.tan(0.5 * fov_y_rad))
    else:
        fov_x_rad = math.radians(max(1.0, min(179.0, fov_x_deg)))
        cam_data.sensor_fit = "HORIZONTAL"
        cam_data.lens = cam_data.sensor_width / (2.0 * math.tan(0.5 * fov_x_rad))

    # Scene render resolution follows background image to keep 1:1 overlay.
    if image_w and image_h:
        bpy.context.scene.render.resolution_x = int(image_w)
        bpy.context.scene.render.resolution_y = int(image_h)
...
class NM_OT_install_da3_dependencies(bpy.types.Operator):
    bl_idname = "nm.install_da3_dependencies"
    bl_label = "Install Depth Anything 3 Dependencies"
    bl_description = "Install Depth Anything 3 (DA3) dependencies next to the add-on into _deps"
    bl_options = {"REGISTER"}
    ...
    def execute(self, context):
        try:
            ...
            requirements = addon_dir / "requirements_da3.txt"

            deps_dir.mkdir(parents=True, exist_ok=True)
            cache_dir.mkdir(parents=True, exist_ok=True)

            if not requirements.exists():
                requirements.write_text("# Optional Depth Anything 3 deps\n", encoding="utf-8")
            ...
            for step in steps:
                ...
                if proc.returncode != 0:
                    ...
                    errolog_path = nm_logging.append_errolog(
                        f"DA3 dependency install failed (exit={proc.returncode}). Terminal log: {terminal_path}",
                        project_root=None,
                    )
                    self.report({"ERROR"}, f"DA3 install failed. See {errolog_path.name}.")
                    return {"CANCELLED"}

            marker = nm_deps_da3.mark_installed(requirements_sha256=nm_deps_da3.requirements_hash(requirements))
            nm_logging.append_terminal("".join(combined_out), project_root=None)
            self.report({"INFO"}, f"DA3 dependencies installed. Marker: {marker.name}")
            return {"FINISHED"}
        except Exception:
            errolog_path = nm_logging.append_errolog("Unhandled exception during DA3 dependency install.", project_root=None)
            nm_logging.append_terminal(traceback.format_exc(), project_root=None)
            self.report({"ERROR"}, f"DA3 install failed. See {errolog_path.name}.")
            raise


class NM_OT_da3_seed(bpy.types.Operator):
    bl_idname = "nm.da3_seed"
    bl_label = "Depth Anything 3 Seed (Mesh/Point Cloud)"
    bl_description = "Run Depth Anything 3 on the selected image and import a seed artifact (mesh if available)"
    bl_options = {"REGISTER"}
    ...
    fov_x_deg: bpy.props.FloatProperty(name="FOV X (deg)", default=60.0, min=1.0, max=179.0)
    target_median_depth: bpy.props.FloatProperty(name="Target Median Depth", default=2.0, min=0.01, max=1000.0)
    max_edge: bpy.props.IntProperty(name="Max Edge", default=1024, min=256, max=4096)
    ...
    def poll(cls, context):
        ...
        if not nm_deps.get_deps_status().installed:
            return False
        if not nm_deps_da3.get_deps_status().installed:
            return False
    ...
    def invoke(self, context, event):
        ...
        model_id = str(getattr(context.scene, "nm_da3_model_id", self.model_id) or self.model_id)
        device = str(getattr(context.scene, "nm_da3_device", self.device) or self.device)
        max_edge = int(getattr(context.scene, "nm_da3_max_edge", self.max_edge) or self.max_edge)
        fov_x_deg = float(getattr(context.scene, "nm_da3_fov_x_deg", self.fov_x_deg) or self.fov_x_deg)
        target_median_depth = float(
            getattr(context.scene, "nm_da3_target_median_depth", self.target_median_depth) or self.target_median_depth
        )
        mesh_stride = int(getattr(context.scene, "nm_da3_mesh_stride", self.stride) or self.stride)
        points_stride = int(getattr(context.scene, "nm_da3_points_stride", self.stride) or self.stride)
        max_points = int(getattr(context.scene, "nm_da3_points_max_points", self.max_points) or self.max_points)
        export_mode = str(self.export_mode or "mesh")

        out_dir = nm_paths.addon_cache_dir() / "artifacts" / "da3" / str(image_id) / uuid.uuid4().hex
        out_dir.mkdir(parents=True, exist_ok=True)

        run_id = uuid.uuid4().hex
        runs_dir = nm_paths.addon_cache_dir() / "runs"
        stdout_path = runs_dir / f"da3_{run_id}.stdout.txt"
        stderr_path = runs_dir / f"da3_{run_id}.stderr.txt"
        ...
        try:
            ...
            nm_logging.append_errolog(f"Failed to start DA3 backend process: {e}", project_root=None)
            self.report({"ERROR"}, "Failed to start DA3 process. See Errolog.")
            return {"CANCELLED"}
        ...
        try:
            ...
            context.scene.nm_job_label = f"DA3 running ({export_mode})..."
        except Exception:
            pass
        nm_statusbar.set_progress(context=context, running=True, factor=0.0, text=f"DA3 ({export_mode})")
    ...
    def modal(self, context, event):
        ...
        # finished
        nm_statusbar.set_progress(context=context, running=True, factor=0.99, text="DA3 (importing)")
        try:
            self._bp.close_files()
        except Exception:
            pass
        ...
        if int(code) != 0:
            errolog_path = nm_logging.append_errolog(
                f"DA3 seed failed (exit={code}). See Terminal.txt.",
                project_root=None,
            )
            self._finish(context)
            self.report({"ERROR"}, f"DA3 seed failed. See {errolog_path.name}.")
            return {"CANCELLED"}

        try:
            payload = self._parse_last_json(stdout_text)
        except Exception:
            nm_logging.append_errolog("DA3 seed returned non-JSON output.", project_root=None)
            self._finish(context)
            self.report({"ERROR"}, "DA3 seed returned invalid output. See logs.")
            return {"CANCELLED"}
        ...
        try:
            ...
            if not import_path:
                raise RuntimeError("DA3 seed did not return output path.")
            ...
            engine_coll = _ensure_generation_collection(_DA3_COLLECTION_NAME)
            ...
            try:
                if str(self._export_mode) == "mesh":
                    ...
                    if mesh_obj is not None:
                        img_path = None
                        # Use the original image for texturing (full resolution / not DA3-preprocessed).
            ...
            nm_logging.append_errolog(f"DA3 import/setup failed: {e}", project_root=None)
            nm_logging.append_terminal(traceback.format_exc(), project_root=None)
            self._finish(context)
            self.report({"ERROR"}, "DA3 import/setup failed. See logs.")
            return {"CANCELLED"}

        self._finish(context)
        self.report({"INFO"}, "DA3 seed imported.")
        return {"FINISHED"}


def register():
    bpy.utils.register_class(NM_OT_install_da3_dependencies)
    bpy.utils.register_class(NM_OT_da3_seed)


def unregister():
    bpy.utils.unregister_class(NM_OT_da3_seed)
    bpy.utils.unregister_class(NM_OT_install_da3_dependencies)
